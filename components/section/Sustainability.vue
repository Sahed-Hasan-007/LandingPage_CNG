    ...
    <div class="max-w-full mx-auto">
      <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 items-center lg:gap-16">
        <div class="flex flex-col items-center">
          ...
          <div class="flex gap-4 mt-8 items-center justify-center md:flex w-[87px] h-[53px]">
            <button ref="prev" @click="handPrev"
              class="custom-prev opacity-100 hover:bg-green-600 hover:text-white px-4 pt-2 rounded-full transition duration-700 ease-in-out"
              :class="{ 'bg-green-600 text-white': activeSlideIndex.value === 0 }"
              :disabled="activeSlideIndex.value === 0">
              <Icon name="ph:arrow-left" size="24" />
            </button>

            <button ref="next" @click="handNext"
              class="custom-next opacity-100 hover:bg-green-600 hover:text-white px-4 pt-2 rounded-full transition duration-700 ease-in-out"
              :class="{ 'bg-green-600 text-white': activeSlideIndex.value === totalSlides.value - 1 }"
